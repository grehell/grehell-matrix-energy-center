...
Do profilu kiosku można dodać inne strony z tego samego Home Assistanta. W polu ścieżki wpisz na przykład `/dashboard-jsbd/deye-kiosk` albo pełny adres `https://grehell.ovh/dashboard-jsbd/deye-kiosk`. Pełny adres zostanie zapisany jako ścieżka lokalna. Adres z innej domeny jest celowo odrzucany, ponieważ sesja Home Assistant i osadzanie strony nie byłyby bezpieczne ani niezawodne.
...
Dane z wcześniejszych wersji są automatycznie uzupełniane do schematu v8. Dotychczasowe encje, dymki, wykresy i profile kiosku zachowują konfigurację, a nowe sceny i nagłówki otrzymują bezpieczne wartości domyślne. Nie usuwaj pliku `.storage/matrix_energy_center`. Po aktualizacji wykonaj pełny restart Home Assistant oraz twarde odświeżenie panelu (`Ctrl+F5`), ponieważ v8.0.0 zawiera nowy plik frontendowy.
