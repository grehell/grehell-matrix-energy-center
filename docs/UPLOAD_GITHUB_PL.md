# Jak poprawnie opublikować v8.0.4 na GitHub

1. Rozpakuj `matrix-energy-center-repository-v8.0.4.zip` na komputerze.
...
4. Zatwierdź wszystkie pliki jednym commitem, np. `Release v8.0.4`.
5. Sprawdź na gałęzi `main`, czy plik
   `custom_components/matrix_energy_center/manifest.json` ma wersję `8.0.4`
   oraz czy istnieje plik
   `custom_components/matrix_energy_center/frontend/matrix-energy-flow-card.js`.
6. Dopiero po tym utwórz nowy tag i wydanie GitHub `v8.0.4` z bieżącej gałęzi
   `main`. Nie wskazuj wcześniejszego commita.
7. W HACS wybierz ponowne pobranie informacji o repozytorium, zainstaluj v8.0.4,
