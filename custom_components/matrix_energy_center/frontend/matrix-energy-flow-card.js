...
class MatrixEnergyFlowCard extends HTMLElement {
  ...
  _customBubble(item) {
    ...
    const icon = item.show_icon === false ? "" : item.icon_type === "emoji"
      ? `<span class="bubble-emoji" aria-hidden="true">${this._esc(item.emoji || "⚡")}</span>`
      : `<ha-icon icon="${this._escAttr(item.icon || "mdi:information-outline")}"></ha-icon>`;
    return `<article class="bubble custom" style="--accent:${color};--bubble-icon-size:${Math.max(12, Math.min(48, Number(item.icon_size || 22)))}px" ${item.entity_id ? `data-entity="${this._escAttr(item.entity_id)}"` : ""}>${icon}<div><small>${this._esc(item.name)}</small><strong>${this._esc(value.formatted)} <i>${this._esc(value.unit)}</i></strong>${related ? `<div class="related">${related}</div>` : ""}</div></article>`;
  }
  ...
  _flowIcon(key, fallback) {
    const style = this._flowSettings(key) || {};
    if (style.icon_type === "emoji") return `<span class="flow-custom-emoji" aria-hidden="true">${this._esc(style.emoji || "⚡")}</span>`;
  ...
  _styles() { return `
    ...
    .flow-custom-image{display:block;width:var(--custom-icon-size,28px);height:var(--custom-icon-size,28px);object-fit:contain;filter:drop-shadow(0 0 7px var(--custom-icon,var(--accent)))}.flow-custom-emoji{display:grid;place-items:center;width:var(--custom-icon-size,28px);height:var(--custom-icon-size,28px);font-size:var(--custom-icon-size,28px);line-height:1;filter:drop-shadow(0 0 7px var(--custom-icon,var(--accent)))}
    .node[data-flow-element]>small,.extra[data-flow-element]>span:not(.flow-custom-emoji)>b{color:var(--custom-name)}.node[data-flow-element]>strong,.extra[data-flow-element]>span:not(.flow-custom-emoji)>strong{color:var(--custom-value)}.extra[data-flow-element]>span:not(.flow-custom-emoji)>small{color:var(--custom-unit)}
